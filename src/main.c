...
int main(int argc, char* argv[]) {
...
#endif
    DLIRS* dlirs = dlirs_create(AM_ALLOCATOR_ARG 8, 4, 0.1f);
    if (dlirs == NULL) {
    ...
    for (int i = 0; i < 10; i++) {
        for (int j = 1; j < i + 1; j++) {
            ...
            int requestResult;
            if ((requestResult = dlirs_request(AM_ALLOCATOR_ARG dlirs, to_store[i], &values[i], &evicted)) == -1) {
                ...
                dlirs_destroy(AM_ALLOCATOR_ARG dlirs);
                return 1;
            }
